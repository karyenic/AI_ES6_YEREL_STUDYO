...
# app.py iÃ§indeki arama bloÄŸunu BeautifulSoup tabanlÄ± canlÄ± site kazÄ±ma motoruyla gÃ¼ncelliyoruz
        ...
        try:
            ...
            if url_match:
                ...
                if r_scrape.status_code == 200:
                    ...
                    if links:
                        web_context = f"[CANLI WEB KAZIMA SONUÃ‡LARI ({target_url})]:\\n" + "\\n".join(links[:50])
                    else:
                        web_context = f"[UYARI: {target_url} tarandÄ± ancak sayfada hiÃ§ baÄŸlantÄ± bulunamadÄ±.]"
                else:
                    web_context = f"[UYARI: {target_url} adresi {r_scrape.status_code} durum koduyla yanÄ±t verdi.]"
        except Exception as e:
            web_context = f"[UYARI: Web tarama hatasÄ±: {e}]" """

if start_idx != -1 and end_idx != -1:
    content = content[:start_idx] + new_snippet + "\n" + content[end_idx:]
    with open(path, "w", encoding="utf-8") as f:
        f.write(content)
    print("BAÅžARILI: app.py canlÄ± BeautifulSoup kazÄ±ma motoruyla gÃ¼ncellendi.")
else:
    print("HATA: Ã‡apa noktalarÄ± bulunamadÄ±.")
